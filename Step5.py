...
def add_new_objects():
    for name, props, position in new_objects:           # получаю тройку нового объекта
        for obj2 in get_objects_by_coords(position):    # для объектов с координатами как у нового объекта
            if game_objects[obj2]['interactable']:      # если есть объекты с interactible == True
